...
describe('CodexProvider error events', () => {
  ...
  it('includes additional details from turn.failed event when present', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
      ...
      runStreamed: () => ({
        events: (async function* () {
          yield {
            type: 'turn.failed',
            error: {
              message: 'Turn failed upstream',
              additional_details: 'The model rejected the final turn payload.'
            }
          };
        })(),
      }),
    ...
    assert.equal(
      errorEvent!.data,
      'Turn failed upstream\n\nThe model rejected the final turn payload.',
    );
  });
